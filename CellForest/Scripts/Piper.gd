...
onready var sound_pluck = load("res://Sound/pluck.wav")
...
func addSharp(data_address, sound, sensitivity = 1, volume = 1, scale = black):
	sharps.append([sound, data_address, sensitivity, scale, -10000])
...
func _ready():
	...
	addStream(S_GROW, sound_cello, 4, 3)
	#addStream(S_LIGHT, sound_oboe, 2)
	addStream(S_SPROUT, sound_digeridoo, 4, 3) # very jittery
	addStream(S_DIE, sound_clarinet, 2)
	...
	# addSharp(B_LIGHT, sound_pluck)
	addSharp(B_LIGHT, sound_glass)
...
func play(st):
	...
	for stream in sharps:
		...
		var sound = stream[0]
		var scale = stream[3]
		var player = Panner.new(stream[0], stream[3], 1);
		add_child(player)
